...
const Pagination = ({ pageNumber, term, totalPages, setLoading, api }) => {
  ...
  const chnagePageData = async (page) => {
    ...
    const options = {
      method: "GET",
      url: `https://api.themoviedb.org/3/search/movie?query=${term}&include_adult=false&language=en-US&page=${page}`,
      headers: {
    ...
    try {
      ...
      navigate(`/search`, { state: { id: res.data, term: term } });
    } catch (error) {
      console.log(error);
    }
  };
